...
public class Factory extends SolidPlatform implements Generator {

	GameObject obj;
	ArrayList<GameObject> generated;
	ArrayList<GameObject> aliveGenerated;
	public int generationDelay;
	int max;
	
	public Factory(double x, double y, double size, GameObject obj, int delay, int max, double range) {
		this(x, y, size, obj, delay, max);
		this.rangeActivated = true;
		this.range = range;
	}
	
	public Factory(double x, double y, double size, GameObject obj, int delay, int max) {
		...
		this.aliveGenerated = new ArrayList<GameObject>();
		this.slipperiness = 0.97;
		this.max = max;
		this.rangeActivated = false;
		this.range = 0;
		
	}
	
	public boolean rangeActivated;
	public double range;
	public int delay = 0;
	@Override
	public void move() {
		aliveGenerated.removeIf(obj -> !GamePanel.getPanel().objects.contains(obj));
		
		if (aliveGenerated.size() < max) {
			...
			if (!occupied && this.delay >= this.generationDelay) {
				if (!rangeActivated || (rangeActivated && distanceTo(GamePanel.getPanel().player) < range )) {
					this.generate();
	...
	public void generate() {
		try {
			...
			this.aliveGenerated.add(newObj);
