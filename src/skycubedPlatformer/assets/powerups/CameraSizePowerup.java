...
public class CameraSizePowerup extends Powerup {
	...
	public CameraSizePowerup(double x, double y, double size, double d) {
		this(x,y,size,null);
		Code code = new Code() {
			@Override
			public void run() {
				GamePanel.target_camera_size = (int) (Main.SIZE*d);
		}};
	...
	public CameraSizePowerup(double x, double y, double size, Code code) {
		super(x, y, size, code);
		this.color = Powerup.COLOR_POWERUP_CAMERASIZE;
		try {this.image = ImageIO.read(this.getClass().getResource("/powerups/camerasize.png"));
		} catch (IOException e) {}
