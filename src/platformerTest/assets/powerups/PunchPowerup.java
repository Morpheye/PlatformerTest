...
public class PunchPowerup extends Powerup {
	
	public PunchPowerup(double x, double y, double size, int amount) {
		this(x,y,size,null);
		Code code = new Code() {
			@Override
			public void run() {
	...
	public PunchPowerup(double x, double y, double size, Code code) {
		super(x, y, size, code);
		this.color = Powerup.COLOR_POWERUP_PUNCH;
		try {this.image = ImageIO.read(this.getClass().getResource("/powerups/punch.png"));
		} catch (IOException e) {}
