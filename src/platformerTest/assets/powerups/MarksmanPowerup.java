...
public class MarksmanPowerup extends Powerup {
	
	public MarksmanPowerup(double x, double y, double size, int amount) {
		this(x,y,size,null);
		Code code = new Code() {
			@Override
			public void run() {
				GamePanel.player.rangedAttackDamage += amount;
			}
		};
	...
	public MarksmanPowerup(double x, double y, double size, Code code) {
		super(x, y, size, code);
		this.color = Powerup.COLOR_POWERUP_MARKSMAN;
		try {this.image = ImageIO.read(this.getClass().getResource("/powerups/marksman.png"));
		} catch (IOException e) {}
