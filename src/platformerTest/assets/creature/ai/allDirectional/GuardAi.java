package platformerTest.assets.creature.ai.allDirectional;
...
public class GuardAi extends CreatureAi {
	...
	public void run(Creature c) {
		...
		if (c.y - this.postY > yBoundMax) { //too far up
			...
			c.movingDown = false;
			c.movingUp = true;
		} else if (target != null && Math.abs(target.y - this.postY) < yBoundMax && Math.abs(target.y - this.postY) > yBoundMin) { //target within guard range
			if (c.y - target.y > 0 && Math.abs(target.x - this.postX) < xBoundMax) {
				c.movingDown = true;
				c.movingUp = false;
			} else if (c.y - target.y < 0 && Math.abs(target.x - this.postX) < xBoundMax) {
				c.movingUp = true;
				c.movingDown = false;
			} else {
				c.movingUp = false;
